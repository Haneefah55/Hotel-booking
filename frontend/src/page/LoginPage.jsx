...
const LoginPage = () =>{
  ...
  const handleOwnerLogin = async(e, role) =>{
    e.preventDefault()
    try{
      await ownerLogin (guestEmail, guestPassword, role)
      handleOpen()
      setTimeout(() =>{
  ...
  const handleGuestLogin = async(e, role) =>{
    e.preventDefault()
    try{
  ...
  return(
    ...
    <div className="w-full h-screen md:h-screen md:max-w-[700px] flex items-center justify-center">
      ...
      <div className="p-6 flex flex-col items-center overflow-hidden relative justify-center">
        ...
        <div className="w-[300px] h-[350px] flex overflow-x-hidden snap-x snap-mandatory gap-10  scroll-smooth">
          <div className={`w-full h-full ${show ? "hidden" : "flex"} bg-gray-100 flex-col   items-center rounded-lg shadow-md transition linear duration-040 flex-shrink-0`}>
            ...
            <form className=" text-amber-800 w-full mx-4 md:p-6 p-4 py-8 text-left text-sm " onSubmit={() =>(handleHostLogin("owner"))}>
              ...
              <p class="text-center mt-4">Don't have an account? <Link to="/account/signup" class="text-blue-500 underline">Sign Up</Link></p>
            </form>
          </div>
          <div className={`w-full h-full flex-col flex-shrink-0 ${show ? "flex" : "hidden"} transition linear duration-040 items-center bg-gray-100`}>
            ...
            <form onSubmit={() =>(handleGuestLogin("guest"))} className=" text-amber-800 w-full mx-4 md:p-6 p-4 py-8 text-left text-sm ">
      ...
      <div className={`bg-gray-100 ${open ? "flex" : "hidden"} space-x-3 p-3 text-sm rounded w-[300px] justify-between items-center absolute top-[50%] border border-gray-300/60`}>
