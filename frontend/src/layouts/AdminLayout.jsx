...
const AdminLayout = () =>{const [isOpen, setIsOpen] = useState(false)
  ...
  return(
    <div className="w-screen">
      <div className="w-full h-[200px] md:h-[150px] bg-[url(./assets/images/hero.png)] flex bg-no-repeat bg-cover items-center justify-center relative ">
        <div className="w-full h-full pt-20 flex justify-center text-gray-200 pl-5 flex-col bg-black/30" >
          <Link to="/" className=" flex gap-2 font-semithin mt-20 md:ml-20 "><House /> Home</Link>
          <h2 className="text-3xl text-center font-semibold mb-10 text-gray-200">Admin Dashboard</h2>
      
        </div>
      
      </div>
      <button className="absolute z-20 top-[38%] right-[85%] md:hidden text-amber-800" onClick={handleOpen}>
      ...
      <div className=" mx-1 md:mx-7 flex items-center mt-10 h-screen relative justify-center w-full">
        ...
        <main className={`flex-1 mt-10 overflow-y-auto p-5 h-screen ${!isOpen ? "w-full" : ""}items-center justify-center`}>
          <Outlet />
        </main>
      </div>
